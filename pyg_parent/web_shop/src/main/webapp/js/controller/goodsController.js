...
app.controller('goodsController' ,function($scope,$controller,$location,typeTemplateService ,itemCatService,uploadService ,goodsService){	
    ...
    // 显示状态
    $scope.is_marketable = ["未上架","已上架"];

    // 审核的方法:
    $scope.updateIs_marketable = function(is_marketable){
        goodsService.updateIs_marketable($scope.selectIds,is_marketable).success(function(response){
            if(response.flag){
                $scope.reloadList();//刷新列表
                $scope.selectIds = [];
            }else{
